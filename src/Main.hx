...
class Main extends Model {
	...
	public function editCell( c : Column, v : JQuery, sheet : Sheet, index : Int ) {
		...
		var obj = v.data("object");
		var val : Dynamic = v.data("value");
		var old = val;
	...
	function columnTypeToClassName( type: ColumnType ) : String {
		return Type.enumConstructor(type).substr(1).toLowerCase();
	}
	...
	function createValueCell( columnIndex: Int, rowIndex: Int, sheet: Sheet, line: JQuery, column: Column, object: Dynamic) : JQuery {
		// var column = sheet.columns[columnIndex];
		// var object = sheet.lines[rowIndex];
		var val : Dynamic = Reflect.field(object,column.name);
		var ctype = "t_" + columnTypeToClassName(column.type);
		var valueCell = J("<td>").addClass(ctype).addClass("c");
		valueCell.data("object", object);
		valueCell.data("value", val);
		// var l = lines[rowIndex];
		// v.appendTo(l);

		updateClasses(valueCell, column, val);

		var html = valueHtml(column, val, sheet, object);
		if( html == "&nbsp;" ) valueCell.text(" ") else if( html.indexOf('<') < 0 && html.indexOf('&') < 0 ) valueCell.text(html) else valueCell.html(html);
		valueCell.data("rowIndex", columnIndex);
		valueCell.click(function(e) {
			if( inTodo ) {
				// nothing
			} else if( e.shiftKey && cursor.s == sheet ) {
				cursor.select = { x : columnIndex, y : rowIndex };
				updateCursor();
				e.stopImmediatePropagation();
			} else
				setCursor(sheet, columnIndex, rowIndex);
			e.stopPropagation();
		});
		...
		function set(val2:Dynamic) {
			...
			val = val2;
			if( val == null )
				Reflect.deleteField(object, column.name);
			else
				Reflect.setField(object, column.name, val);
			html = valueHtml(column, val, sheet, object);
			valueCell.html(html);
		...
		switch( column.type ) {
		case TImage:
			valueCell.find("img").addClass("deletable").change(function(e) {
				if( Reflect.field(object,column.name) != null ) {
					Reflect.deleteField(object, column.name);
					refresh();
					save();
				}
			...
			valueCell.dblclick(function(_) editCell(column, valueCell, sheet, rowIndex));
			valueCell[0].addEventListener("drop", function(e : js.html.DragEvent ) {
				e.preventDefault();
				e.stopPropagation();
				if (e.dataTransfer.files.length > 0) {
					untyped valueCell.dropFile = e.dataTransfer.files[0].path;
					editCell(column, valueCell, sheet, rowIndex);
					untyped valueCell.dropFile = null;
				}
			});
		case TList:
			var key = sheet.getPath() + "@" + column.name + ":" + rowIndex;
			valueCell.click(function(e) {
				var next = line.next("tr.list");
				if( next.length > 0 ) {
					if( next.data("name") == column.name ) {
						next.change();
						return;
					}
					next.change();
				}
				next = J("<tr>").addClass("list").data("name", column.name);
				J("<td>").appendTo(next);
				...
				if( !inTodo )
					div.hide();
				var content = J("<table>").appendTo(div);
				var psheet = sheet.getSub(column);
				if( val == null ) {
					val = [];
					Reflect.setField(object, column.name, val);
				}
				psheet = new cdb.Sheet(base,{
					...
					lines : val, // ref
					separators : [], // none
				},key, { sheet : sheet, column : columnIndex, line : rowIndex });
				fillTable(content, psheet);
				next.insertAfter(line);
				valueCell.text("...");
				openedList.set(key,true);
				next.change(function(e) {
					if( column.opt && val.length == 0 ) {
						val = null;
						Reflect.deleteField(object, column.name);
						save();
					}
		...
		case TProperties:
			...
			valueCell.click(function(e) {
				...
				openedList.set(key,true);
				next.change(function(e) {
					if( column.opt && Reflect.fields(val).length == 0 ) {
						val = null;
						Reflect.deleteField(object, column.name);
						save();
					}
		...
		case TLayer(_):
			// nothing
		case TFile:
			valueCell.find("input").addClass("deletable").change(function(e) {
				if( Reflect.field(object,column.name) != null ) {
					Reflect.deleteField(object, column.name);
					refresh();
					save();
		...
		case TTilePos:
			...
			valueCell.find("div").addClass("deletable").change(function(e) {
				if( Reflect.field(object,column.name) != null ) {
					Reflect.deleteField(object, column.name);
					refresh();
					save();
		...
		default:
			valueCell.dblclick(function(e) editCell(column, valueCell, sheet, rowIndex));
		}
	...
	function fillTree( content: JQuery, items: Array<TreeItem>, sheet: Sheet, columnIndex: Int, rowIndex: Int ) {
		for( item in items ) {
			...
			if (item.children.length > 0){
				...
				childrenRow.append(J("<td>").attr("colspan", colCount).append(childrenTable));

				fillTree(childrenTable, item.children, sheet, columnIndex, rowIndex);
			}
		}
